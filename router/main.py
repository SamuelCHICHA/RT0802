...
def routing(sock_src: socket.socket, ip_address_src: str, ip_address_dest: str):
    ...
    sock_dest.connect((ip_address_dest, 53))
...
def main():
    ...
    while True:
        ...
        router_logger.info(f"Accepting new connection from {address_src[0]}")
        client_src = next(filter(lambda client: client['ip'] == address_src[0], accepted_clients), None)
        if client_src is not None:
            # router_logger.debug(f"{client_src['ip']} is authorized")
            client_dest = next(filter(lambda client: client['ip'] != address_src[0], accepted_clients), None)
