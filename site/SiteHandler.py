...
class SiteHandler(socketserver.StreamRequestHandler):
    """Class meant to represent the handler for the threaded tcp server"""
    CERTICATE_REQUEST = 1
    ...
    def handle(self) -> None:
        ...
        # Splitting data according to format
        ...
        if code == self.MESSAGE:
            # We decrypt the message with the symmetric key
            ...
            try:
                ...
                if self.server.check_certificate(certificate):
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                        ...
                        # Sending the encrypted (with RSA) symmetric key (AES) and the message encrypted with it
            ...
            if decrypted_message == "Ping":
                # If the message is ping it answers by (encrypted and encoded) 'pong'
